...
template <typename T> void test(ArrayList<T> testobj);
int main()
{
	ArrayList<int> a;
	ArrayList<double> b;
	...
	ArrayList<int> c={23,45,23,123,56,23};
	ArrayList<double> d={32.23,324.2334,876.323456,3213.0983,998.9645};
	c.print();
	d.print();
...
template <typename T> void test(ArrayList<T> testobj)
{
	...
	try{
		...
		cout<<"There are "<<testobj.length()<<" element(s) in the default initialization object."<<endl;
		cout<<"There are :";
		testobj.printlink();
		testobj.print();
		...
		cout<<"Now testing push_front(T x) function:"<<endl;
		for(unsigned char i=0;i<10;++i)
		{
			temp=u(e);
			testobj.push_front(temp);
			cout<<"Generated random number: "<<temp<<". Put it into the list."<<endl;
			cout<<"Now the list is:"<<endl;
			testobj.printlink();
			testobj.print();
		...
		{
			if(temp<0) break;
			typename ArrayList<T>::iter tempiter=testobj.search(temp);
			if(tempiter==-1)
				throw runtime_error("Not found the location in the list, try again!");
			else
				{
				temp=u(e);
				cout<<"Generated random number: "<<temp<<". Put it before "<<testobj.key_value(tempiter)<<endl;
		...
		typename ArrayList<T>::size_type number;
		cin>>number;
		do
		{
			...
			if(tempiter==-1)
			...
			else
				{
					...
					{
					temp=u(e);
					cout<<"Generated random number: "<<temp<<". Put "<<number<<" of them before "<<testobj.key_value(tempiter)<<endl;
